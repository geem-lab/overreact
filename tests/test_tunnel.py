...
def test_wigner_tunneling_corrections_are_correct() -> None:
    """Ensure Wigner tunneling values are correct."""
    with pytest.raises(ValueError, match="vibfreq should not be zero for tunneling"):
...
def test_eckart_tunneling_corrections_are_correct() -> None:
...
def test_eckart_is_symmetric() -> None:
...
def test_low_level_eckart_against_johnston1962() -> None:
    ...
    for u, gamma in zip(us, gammas, strict=False):
    ...
    for u, gamma in zip(us[:-1], gammas, strict=False):
    ...
    for u, gamma in zip(us[:-2], gammas, strict=False):
