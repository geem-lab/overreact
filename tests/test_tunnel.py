...
def test_low_level_eckart_against_johnston1962():  # noqa: C901, PLR0912, PLR0915
    ...
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 0.5) == pytest.approx(gamma, 2e-2)  # noqa: SLF001
    ...
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1) == pytest.approx(gamma, 2e-2)  # noqa: SLF001

    gammas = [1.21, 1.35, 1.51, 1.71, 1.93, 2.50, 3.26, 4.28, 7.48]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1, 2) == pytest.approx(gamma, 6e-3)  # noqa: SLF001

    gammas = [1.14, 1.24, 1.37, 1.53, 1.71, 2.16, 2.78, 3.60, 6.16]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1, 4) == pytest.approx(gamma, 5e-3)  # noqa: SLF001

    gammas = [1.08, 1.16, 1.26, 1.39, 1.54, 1.92, 2.43, 3.12, 5.25]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1, 8) == pytest.approx(gamma, 5e-3)  # noqa: SLF001

    gammas = [1.06, 1.12, 1.21, 1.33, 1.46, 1.81, 2.28, 2.91, 4.88]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1, 12) == pytest.approx(gamma, 9e-3)  # noqa: SLF001

    gammas = [1.04, 1.10, 1.18, 1.29, 1.42, 1.75, 2.20, 2.80, 4.66]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1, 16) == pytest.approx(gamma, 5e-3)  # noqa: SLF001

    gammas = [1.03, 1.08, 1.16, 1.26, 1.39, 1.70, 2.14, 2.72, 4.52]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 1, 20) == pytest.approx(gamma, 7e-3)  # noqa: SLF001
    ...
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 2) == pytest.approx(gamma, 4e-3)  # noqa: SLF001

    gammas = [1.26, 1.47, 1.77, 2.16, 2.66, 4.20, 6.85, 11.4, 33.4]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 2, 4) == pytest.approx(gamma, 5e-3)  # noqa: SLF001

    gammas = [1.19, 1.36, 1.61, 1.93, 2.36, 3.65, 5.87, 9.69, 28.0]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 2, 8) == pytest.approx(gamma, 4e-3)  # noqa: SLF001

    gammas = [1.16, 1.32, 1.54, 1.84, 2.23, 3.41, 5.44, 8.94, 25.6]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 2, 12) == pytest.approx(gamma, 5e-3)  # noqa: SLF001

    gammas = [1.14, 1.29, 1.50, 1.78, 2.15, 3.27, 5.20, 8.51, 24.2]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 2, 16) == pytest.approx(gamma, 4e-3)  # noqa: SLF001

    gammas = [1.12, 1.27, 1.47, 1.74, 2.10, 3.18, 5.03, 8.22, 23.3]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 2, 20) == pytest.approx(gamma, 4e-3)  # noqa: SLF001

    gammas = [1.30, 1.58, 2.02, 2.69, 3.69, 7.60, 17.3, 42.4, 304]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 4, 4) == pytest.approx(gamma, 8e-3)  # noqa: SLF001

    gammas = [1.25, 1.51, 1.93, 2.56, 3.56, 7.57, 18.0, 46.7, 376]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 4, 8) == pytest.approx(gamma, 5e-3)  # noqa: SLF001

    gammas = [1.22, 1.47, 1.86, 2.46, 3.39, 7.16, 17.0, 44.0, 354]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 4, 12) == pytest.approx(gamma, 3e-3)  # noqa: SLF001

    gammas = [1.20, 1.44, 1.81, 2.39, 3.28, 6.88, 16.2, 41.9, 335]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 4, 16) == pytest.approx(gamma, 3e-3)  # noqa: SLF001

    gammas = [1.19, 1.42, 1.78, 2.34, 3.20, 6.68, 15.7, 40.3, 321]
    for u, gamma in zip(us, gammas):
        assert rx.tunnel._eckart(u, 4, 20) == pytest.approx(gamma, 2e-3)  # noqa: SLF001
    ...
    for u, gamma in zip(us[:-1], gammas):
        assert rx.tunnel._eckart(u, 8, 8) == pytest.approx(gamma, 2e-2)  # noqa: SLF001

    gammas = [1.22, 1.54, 2.04, 2.96, 4.68, 15.4, 71.7, 445]
    for u, gamma in zip(us[:-1], gammas):
        assert rx.tunnel._eckart(u, 8, 12) == pytest.approx(gamma, 2e-2)  # noqa: SLF001

    gammas = [1.21, 1.53, 2.02, 2.93, 4.65, 15.6, 74.4, 473]
    for u, gamma in zip(us[:-1], gammas):
        assert rx.tunnel._eckart(u, 8, 16) == pytest.approx(gamma, 2e-2)  # noqa: SLF001

    gammas = [1.20, 1.51, 2.00, 2.90, 4.61, 15.5, 74.2, 474]
    for u, gamma in zip(us[:-1], gammas):
        assert rx.tunnel._eckart(u, 8, 20) == pytest.approx(gamma, 2e-2)  # noqa: SLF001
