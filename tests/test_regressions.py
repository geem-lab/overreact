...
def test_basic_example_for_solvation_equilibria() -> None:
    ...
    for qrrho in [False, (False, True), True]:
        ...
        assert delta_freeenergies[2] / constants.kcal == pytest.approx(
            -6.70 + concentration_correction / constants.kcal,
            1.5e-1,
        )
...
def test_basic_example_for_solvation_phase_kinetics() -> None:
    ...
    assert delta_freeenergies + sym_correction == pytest.approx(
        [9.8, 9.9, 10.1, 10.4, 10.6, 10.9, 11.2],
        8e-3,
    )
    ...
    assert np.log10(k) == pytest.approx(
        np.log10([3.3e6, 3.4e6, 4.0e6, 4.7e6, 5.5e6, 6.4e6, 7.3e6]),
        6e-3,
    )
    ...
    assert delta_freeenergies == pytest.approx(
        [7.9, 7.9, 8.1, 8.3, 8.5, 8.7, 8.8],
        7e-3,
    )
    k = rx.rates.eyring(delta_freeenergies * constants.kcal, temperature=temperatures)
    assert k == pytest.approx([9.8e6, 1.0e7, 1.2e7, 1.4e7, 1.7e7, 1.9e7, 2.2e7], 8e-2)
    assert np.log10(k) == pytest.approx(
        np.log10([9.8e6, 1.0e7, 1.2e7, 1.4e7, 1.7e7, 1.9e7, 2.2e7]),
        5e-3,
    )
    ...
    assert np.log10(k) == pytest.approx(
        np.log10([2.3e7, 2.4e7, 2.7e7, 3.0e7, 3.3e7, 3.7e7, 4.1e7]),
        6e-3,
    )
...
def test_basic_example_for_gas_phase_kinetics() -> None:
    ...
    assert 1e16 * k == pytest.approx(
        1e16 * np.array([2.2e-16, 7.5e-15, 7.9e-15, 5.6e-14]),
        7e-2,
    )
    assert np.log10(k) == pytest.approx(
        np.log10([2.2e-16, 7.5e-15, 7.9e-15, 5.6e-14]),
        2e-3,
    )
    ...
    k = rx.rates.convert_rate_constant(k, "cm3 particle-1 s-1", molecularity=2)
    assert 1e16 * k == pytest.approx(
        1e16 * np.array([8.8e-16, 3.0e-14, 3.1e-14, 2.2e-13]),
        8e-2,
    )
    assert np.log10(k) == pytest.approx(
        np.log10([8.8e-16, 3.0e-14, 3.1e-14, 2.2e-13]),
        3e-3,
    )
    ...
    k *= kappa
    assert 1e16 * k == pytest.approx(
        1e16 * np.array([1.5e-14, 1.2e-13, 1.2e-13, 5.1e-13]),
        7e-2,
    )
    assert np.log10(k) == pytest.approx(
        np.log10([1.5e-14, 1.2e-13, 1.2e-13, 5.1e-13]),
        3e-3,
    )
...
def test_logfiles_for_hickel1992() -> None:
    ...
    assert data.vibfreqs == pytest.approx(
        [1065.8, 1621.5, 1620.6, 3500.2, 3615.5, 3617.3],
        4e-2,
    )  # M06-2X/6-311++G(d,p) from doi:10.1002/qua.25686
...
def test_logfiles_for_tanaka1996() -> None:
    ...
    assert data.vibfreqs == pytest.approx(
        [1367, 1367, 1367, 1598, 1598, 3070, 3203, 3203, 3205],
        8e-3,
    )  # UMP2/6-311G(3d,2p) from doi:10.1002/qua.25686
